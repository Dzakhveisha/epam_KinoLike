...
<fmt:bundle basename="pagecontent" prefix="main.">
    ...
    <div id=mainContent>
        <ul id="filtres">
            <li><a href="${pageContext.request.contextPath}/controller?command=show_main"><fmt:message key="all"/> </a>
            </li>
            <c:if test="${not empty requestScope.genres}">
                <fmt:bundle basename="pagecontent" prefix="genre.">
                    <c:forEach var="genre" items="${requestScope.genres}">
                        <li>
                            <a href="${pageContext.request.contextPath}/controller?command=show_main&genre=${genre.name()}"><fmt:message
                                    key="${genre.name()}"/></a>
                        </li>
                    </c:forEach>
                </fmt:bundle>
                ...
                <c:forEach var="film" items="${requestScope.films}">
                    <div class="movieItem">
                        ...
                        <div class="textDescription">
                            <h2>${film.name}</h2>
                            <fmt:bundle basename="pagecontent" prefix="genre.">
                                <a href="${pageContext.request.contextPath}/controller?command=show_main&genre=${film.genre.name()}"><fmt:message
                                        key="${film.genre.name()}"/></a>
                            </fmt:bundle>
